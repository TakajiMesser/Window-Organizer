...
class ShortcutManager {
    ...
    var shortcutByName = [String: Shortcut]()
    var shortcuts = [Shortcut]()
    var shortcutKeys = Set<UInt16>()
    var globalMonitor: Any?
    
    init() {
        addShortcut(Shortcut("Layout", 0x2F, [NSEvent.ModifierFlags.command, NSEvent.ModifierFlags.shift, NSEvent.ModifierFlags.option], { () in
            LayoutManager.sharedInstance.layoutFor("Main")!.apply()
        }))
    }
    
    func registerGlobalKeyMonitor() {
        globalMonitor = NSEvent.addGlobalMonitorForEvents(matching: [NSEvent.EventTypeMask.keyDown], handler: onKeyDown)
    }
    
    func unregisterGlobalKeyMonitor() {
    ...
    func onKeyDown(event: NSEvent) {
        if shortcutKeys.contains(event.keyCode) {
            for shortcut in shortcuts {
                shortcut.onKeyDown(event.keyCode, event.modifierFlags)
            }
        }
    }
    
    func addShortcut(_ shortcut: Shortcut) {
        shortcutByName[shortcut.name] = shortcut
        shortcuts.append(shortcut)
        shortcutKeys.insert(shortcut.key)
    }
}
